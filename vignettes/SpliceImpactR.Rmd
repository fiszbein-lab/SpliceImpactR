...
elm_features <- get_protein_features(c("elm"), annotation_df$annotations, timeout = 600, test = TRUE)
```
...
protein_feature_total <- get_comprehensive_annotations(list(signalp_features, interpro_features, elm_features))
...
enriched_domains <- enrich_domains_hypergeo(hits_domain, bg)
...
For PPI analysis, we first obtain protein-protein interactions from a premade set derived from BioGrid, PPIDM, and ELM  
...
Identify an event of interest from the differential inclusion results and 
...
You can run the full pipeline in one call and choose either compact `data.table` outputs or a single S4 object.
...
For paired-hit summaries, use either the generic accessor or convenience wrappers:
...
You can inspect schema and slot usage with:
