...
explode_coords <- function(ev, which = c("inc","exc")) {
  ...
  split_vec <- strsplit(ifelse(is.na(ev[[col]]) | !nzchar(ev[[col]]), "", ev[[col]]), ";", fixed = TRUE)
...
match_events_to_annotations_vec <- function(events,
  ...
  # 0) annotations → EX/TX/protein_tx
  ...
  # add human-readable inc_coord if not present
  if (!("inc_coord" %in% names(inc_long))) {
...
#'
#' This is a wrapper around \code{\link{match_events_to_annotations_vec}} that
...
#'   coordinates compatible with \code{match_events_to_annotations_vec}.
#' @param annotations A data.frame of transcript/exon annotation rows.
#'   Typically generated by \code{\link{get_annotations}}.
#' @param chunk_size Integer, number of event rows to process per chunk
#'   (default = 50,000).
#' @param ... Additional arguments passed to \code{match_events_to_annotations_vec}.
#'
...
#' @seealso \code{\link{match_events_to_annotations_vec}},
#'   \code{\link{build_from_annotations}}
#'
#' @importFrom data.table as.data.table rbindlist setorder %chin%
#' @export
