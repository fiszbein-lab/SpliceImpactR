...
#'
#' Fits a quasi-binomial GLM to estimate ΔPSI between two conditions.
...
#' (`psi_adj ~ condition`) to estimate ΔPSI and significance, optionally
...
#' Filter event pairs by significance and ΔPSI thresholds
#'
#' Keeps all rows belonging to events where at least one isoform or site
#' passes adjusted p-value and ΔPSI significance criteria.
#'
...
#' @param dpsi_thr Numeric. Absolute ΔPSI threshold (default `0.1`).
#'
...
#'
#' Generates a volcano plot of ΔPSI vs. –log10(FDR) highlighting significant events.
#'
...
#' shown in light grey. Dashed and dotted lines indicate ΔPSI and FDR thresholds.
#'
#' @examples
#' sample_frame <- data.frame(path = c(check_extdata_dir('rawData/control_S5/'),
#'                                     check_extdata_dir('rawData/control_S6/'),
...
plot_di_volcano_dt <- function(di,
  ...
  p <- ggplot2::ggplot(DT, ggplot2::aes(x = delta_psi, y = -log10(padj))) +
    ...
    ggplot2::labs(
      subtitle = paste0("Significant events (|ΔPSI| ≥ ",
