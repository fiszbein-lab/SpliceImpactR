...
#'
#' @param exon_df A \code{data.frame} or \link[data.table]{data.table} with
...
#' @return A \link[data.table]{data.table} with the subset of exons successfully
...
#'
#' @seealso [build_from_annotations()], [GenomicRanges::findOverlaps()]
#'
...
#'
#' @return A \link[data.table]{data.table} (or tibble-like) with columns
...
#'
#' @return A \link[data.table]{data.table} with domain lists and summary metrics
...
get_domain_background <- function(background, protein_features) {
  ...
  doms <- D[, .(domains = unique(domain_id)), by = transcript_id][
    , .(domains = list(domains)), by = transcript_id]
  ...
  bg[, `:=`(
    domains_1 = Map(setdiff, d1, d2),
    domains_2 = Map(setdiff, d2, d1)
  )]
...
#'   }
#'
#' @seealso [make_transcript_pairs()], [build_tx_length_index()]
#' @keywords internal
#' @noRd
#'
