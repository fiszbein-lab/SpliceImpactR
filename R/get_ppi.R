...
get_ppidm <- function(
  ...
  if (download == TRUE) {
    ...
    q <- BiocFileCache::bfcquery(bfc, url, field = "fpath")
    rid <- if (nrow(q)) q$rid[1] else {
      qn <- BiocFileCache::bfcquery(bfc, name, field = "rname")
      if (nrow(qn)) qn$rid[1] else BiocFileCache::bfcadd(bfc, rname = name, fpath = url)
    }

    BiocFileCache::bfcdownload(bfc, rid, ask = FALSE)  # optional but explicit
    ...
    paths <- paste0(load_dir, "ppidm.csv")
    return(pfam_to_ip(data.table::setDT(do.call(rbind, lapply(paths, fread)))))
  }
}
...
get_ppi_switches <- function(hits_all, ppi) {
  ...
  res <- H[, {
    ...
    if (pc_class == 'protein_coding') {
      inc_ppi <- unique(c(ppi[ensembl_transcript_id == transcript_id_inc, i.ensembl_transcript_id], ppi[i.ensembl_transcript_id == transcript_id_inc, ensembl_transcript_id]))
      exc_ppi <- unique(c(ppi[ensembl_transcript_id == transcript_id_exc, i.ensembl_transcript_id], ppi[i.ensembl_transcript_id == transcript_id_exc, ensembl_transcript_id]))
      inc_only <- setdiff(inc_ppi[inc_ppi != transcript_id_inc], exc_ppi[exc_ppi != transcript_id_exc])
      exc_only <- setdiff(exc_ppi[exc_ppi != transcript_id_exc], inc_ppi[inc_ppi != transcript_id_inc])
