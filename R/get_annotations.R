...
#'
#' @examples
#' strip_ver(c("ENSG00000123456.7", "ENST00000567890.2"))
#' @keywords internal
...
#'
#' @examples
#' prot_hdr_to_enst(">ENSP00000369497.3|ENST00000355832.3|ENSG00000141510.15")
#'
#' @keywords internal
...
load_seq_map <- function(gtf_df, txfa_path, aafa_path,
  ...
  # stable uid for this table (independent of gtf_df’s row_uids)
...
restrict_gtf_genetype <- function(gtf_df,
                                  retrictions = c('protein_coding', 'lncRNA',
                                                  'snoRNA', 'snRNA', 'rRNA',
                                                  'miRNA', 'MT_tRNA', 'MT_rRNA')) {
  return(gtf_df[gtf_df$gene_type %in% retrictions,])
}
...
#' ones follow Bioconductor’s cache convention via
#' [tools::R_user_dir()].
#'
