...
#'   }
#' @param plot Logical; if `TRUE` (default), calls [plot_prox_dist()]
#'   on the resulting classification table.
#'
...
plot_length_comparison <- function(
  ...
  p_box <- ggplot(Dlong, aes(condition, value, fill = condition)) +
    # paired lines
    geom_line(aes(group = id), color = "grey70", size = 0.2, alpha = 0.6,
  ...
  # ---- plot 2: density of Δ length (INC - EXC) ------------------------------
  p_density <- ggplot(Ddiff, aes(diff)) +
    geom_density(fill = "#7F5A83", alpha = 0.5, color = "black") +
    geom_vline(xintercept = 0, linetype = "dashed") +
    labs(x = sprintf("Δ %s Length (INC − EXC)", tools::toTitleCase(mode)),
         y = "Density",
         title = NULL) +
    theme_bw(base_size = 12)
...
plot_alignment_summary <- function(hits,
  ...
  A <- hits[, .(score = get(pid_col), summary_classification)]
  C <- A[, .N, by = summary_classification]
  A <- A[!is.na(score)]
...
#' constructs an exon × sample matrix, and computes size factors:
...
overview_splicing_comparison_fixed <- function(
  ...
  epg_sample_gene <- unique(E[, .(sample, cond_label, gene_id, event_key)])[, .N, by=.(sample, cond_label, gene_id)][sample_df, on = .(sample=sample_name, cond_label=condition)]
...
#' @param sig_delta Absolute ΔHIT threshold for volcano highlighting
#'   (default = 0.2).
#' @param fdr_thresh FDR threshold for volcano significance lines
#'   (default = 0.05).
#' @param seed Random seed for reproducible downsampling of heatmap
#'   events (default = 1).
#'
#' @details
...
compare_hit_index <- function(
    ...
    top_n_heatmap = 5000,         # how many events to show in the heatmap (by |ΔHIT|)
    sig_delta = 0.2,             # volcano threshold for |ΔHIT|
    fdr_thresh = 0.05,
    seed = 1L                    # for reproducible downsampling of heatmap if needed
) {
  ...
  # wilcoxon per event (need >=2 obs per group)
  wcx <- DT[, {
    nC <- sum(group == "control" & !is.na(hit))
    nT <- sum(group == "test"    & !is.na(hit))
    pv <- if (nC >= 0.5*cC && nT >= 0.5*cT) {
      suppressWarnings(wilcox.test(hit[group=="control"],
                                   hit[group=="test"], exact = FALSE)$p.value)
    } else NA_real_
    .(p_value = pv, n_control = nC, n_test = nT)
  }, by = event_key]
  ...
  # heatmap (top |ΔHIT| events, two columns: control, test)
  set.seed(seed)
  ...
  # ---------------------------- plots -------------------------
  # 1) Scatter: control vs test mean HIT, color by |ΔHIT|
  ...
  # 2) Heatmap: two columns (control/test) for top |ΔHIT| events
  p_heat <- ggplot(hm_long, aes(x = group, y = event_key, fill = mean_hit)) +
    ...
    labs(x = NULL, y = NULL, title = "Top |ΔHIT| events (means)") +
    theme_minimal() +
    theme(
...
#' }
#'
#' UpSet plots require the **ComplexUpset** package (optional).
#'
#' @return A named list with:
#' \describe{
...
#' }
#'
#' @seealso [relative_use_pre_post()], [compare_sequence_frame()],
#'   [plot_alignment_summary()], [plot_length_comparison()]
#'
#' @examples
#' sample_frame <- data.frame(path = c(check_extdata_dir('rawData/control_S5/'),
#'                                     check_extdata_dir('rawData/control_S6/'),
...
integrated_event_summary <- function(
  ...
  if (!is.na(gene_col)) {
    # Binary matrix: genes × event types
    ...
    if (length(evt) >= 2) {
      ...
      coord_heatmap <- ggplot(JT, aes(et1, et2, fill = jaccard)) +
        ...
        scale_fill_gradient(low = "white", high = "deeppink4", na.value = "grey95",
                            name = "Jaccard") +
        labs(x = NULL, y = NULL, title = "Event-type coordination across genes") +
        theme_minimal() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1),
              panel.grid = element_blank())
    } else {
      coord_heatmap <- ggplot() + theme_void() + ggtitle("Coordination heatmap (need ≥2 event types)")
    }
  } else {
...
get_enrichment <- function(
  ...
  suppressPackageStartupMessages({
    ...
    if ("Reactome" %in% sources && !requireNamespace("ReactomePA", quietly = TRUE))
      stop("Please install ReactomePA to use Reactome enrichment.")
  ...
  if (nrow(combined_sig)) {
    top <- combined_sig[order(p.adjust, -Count)][1:min(top_n_plot, .N)]
...
#'                      annotations = annotation_df$annotations,
#'                      protein_features = protein_feature_total)
#' enrichment <- get_enrichment(get_domain_gene_for_enrichment(hits_final), bg$gene_id, species = 'human', 'ensembl', 'MSigDB:H')
#' }
...
#' @param delta_psi_threshold Absolute ΔPSI threshold (default `0.1`).
#'
#' @return Character vector of gene IDs.
#' @export
#'
#' @examples
#' \dontrun{
#' di_genes <- get_di_gene_enrichment(di_results)#' sample_frame <- data.frame(path = c(check_extdata_dir('rawData/control_S5/'),
#'                                     check_extdata_dir('rawData/control_S6/'),
#'                                     check_extdata_dir('rawData/control_S7/'),
#'                                     check_extdata_dir('rawData/control_S8/'),
#'                                     check_extdata_dir('rawData/case_S1/'),
...
#'                      annotations = annotation_df$annotations,
#'                      protein_features = protein_feature_total)
#' enrichment <- get_enrichment(get_domain_gene_for_enrichment(res), bg$gene_id, species = 'human', 'ensembl', 'MSigDB:H')
#' }
