...
#'   }
#' @param key_cols Character vector of columns that uniquely identify events.
...
get_pairs <- function(x,
  ...
  # keep only keys that have at least one + and one – ΔPSI
  ...
  # order like combine_inc_exc(): by key (then strongest |ΔPSI| on each side if available)
  dA <- paste0('delta_psi', "_inc")
  dB <- paste0('delta_psi', "_exc")
  if (dA %in% names(out) && dB %in% names(out)) {
    out[, ordA := -abs(get(dA))]
    out[, ordB := -abs(get(dB))]
